...
printf '{"ok":true,"adapter":"discord-companion.sh","mode":"offline-safe","connected":false,"dryRun":%s,"webhookConfigured":%s,"ack":{"posted":false,"presenceUpdated":false,"reason":"safe stub"},"guild":"%s","channel":"%s","presence":"%s","activity":{"template":"%s","title":"%s","detail":"%s"},"onlineMembers":42,"voiceMembers":7,"mentions":1,"actions":[{"id":"discord.postStatus","enabled":true,"dryRun":%s,"target":"#%s"},{"id":"discord.setPresence","enabled":true,"dryRun":%s,"state":"%s"},{"id":"discord.openVoice","enabled":true,"dryRun":%s,"channel":"%s"}],"recentActivity":[{"user":"alex","event":"joined voice","ageMinutes":3},{"user":"sam","event":"posted status","ageMinutes":8},{"user":"riley","event":"mentioned project channel","ageMinutes":12}],"rows":[{"title":"Guild","value":"%s","detail":"channel #%s"},{"title":"Presence","value":"%s","detail":"%s"},{"title":"Voice","value":"7 members","detail":"open voice action dry-run ready"},{"title":"Webhook","value":"%s","detail":"postStatus stays local unless configured"}],"source":"discord-companion.sh"}\n' \
  "$dry_run" "$webhook_configured" "$(json_escape "$guild")" "$(json_escape "$channel")" "$(json_escape "$presence")" "$(json_escape "$activity")" "$(json_escape "$activity_title")" "$(json_escape "$activity_detail")" \
  "$dry_run" "$(json_escape "$channel")" "$dry_run" "$(json_escape "$presence")" "$dry_run" "$(json_escape "$channel")" "$(json_escape "$guild")" "$(json_escape "$channel")" "$(json_escape "$presence")" "$(json_escape "$activity_detail")" "$webhook_configured"
